...
export default function App() {
  ...
  const [transferFromId, setTransferFromId] = useState(null) // null = hidden, accountId = open with pre-selected
  ...
  const handlePayCard = useCallback((cardId) => {
    setData(prev => {
      ...
      const tx = {
        id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
        type: 'card-payment',
        cardId, accountId: card.accountId, amount: amt,
        date: new Date().toISOString(),
      }
  ...
  return (
    <div className="flex min-h-screen bg-[#f8f9fb] font-['Inter',system-ui,sans-serif]">
      {/* Left Sidebar */}
      <aside className={`${collapsed ? 'w-[68px]' : 'w-48'} bg-white border-r border-gray-100 flex flex-col shrink-0 sticky top-0 h-screen transition-all duration-300`}>
        ...
        <nav className={`flex-1 ${collapsed ? 'px-2' : 'px-3'} pt-4`}>
          {NAV_ITEMS.map(item => (
            <button
              ...
              className={`w-full flex items-center ${collapsed ? 'justify-center' : ''} gap-3 ${collapsed ? 'px-0 py-2.5' : 'px-3 py-2.5'} rounded-xl text-sm font-medium mb-1 transition-all cursor-pointer ${
                tab === item.id
        ...
        <div className={`${collapsed ? 'px-2' : 'px-3'} pb-4`}>
          <button
            ...
            className={`w-full flex items-center ${collapsed ? 'justify-center' : ''} gap-3 ${collapsed ? 'px-0 py-2.5' : 'px-3 py-2.5'} rounded-xl text-sm font-medium transition-all cursor-pointer ${
              tab === 'settings'
                ? 'bg-gray-900 text-white shadow-lg shadow-gray-900/20'
                : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'
            }`}
        ...
        <div className="max-w-5xl mx-auto px-8 py-8">
          {tab === 'dashboard' && (
            <Dashboard
              ...
              onTransfer={(accId) => setTransferFromId(accId)}
            />
          )}
          {tab === 'accounts' && (
            <AccountManager
          ...
          {tab === 'cards' && (
            <CardManager cards={data.cards} accounts={data.accounts} onChange={updateCards} onPayCard={handlePayCard} />
          )}
          {tab === 'settings' && (
...
function SettingsPage({ data, setData }) {
  ...
  const toggleFaceId = async () => {
    if (faceIdOn) {
      removeFaceId()
      setFaceIdOn(false)
      setSetupMsg('已關閉生物辨識鎖定')
    } else {
      try {
        setSetupMsg('設定中...')
        await setupFaceId()
        setFaceIdOn(true)
        setSetupMsg('已啟用生物辨識鎖定')
      } catch (err) {
        setSetupMsg(err.message || '設定失敗')
      }
    }
      ...
      <div className="space-y-4 max-w-lg">
          ...
          <div className="flex items-center gap-4">
            ...
            {faceIdSupported ? (
              <button
                ...
                className={`w-12 h-7 rounded-full transition-colors cursor-pointer relative ${faceIdOn ? 'bg-indigo-500' : 'bg-gray-200'}`}
              >
                <div className={`w-5 h-5 rounded-full bg-white shadow-sm absolute top-1 transition-all ${faceIdOn ? 'left-6' : 'left-1'}`} />
              </button>
            ) : (
              <span className="text-xs text-gray-400">不支援</span>
            )}
          </div>
          {setupMsg && <p className="text-xs text-indigo-500 mt-2 ml-14">{setupMsg}</p>}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="font-medium text-gray-800 text-sm">資料儲存</div>
              <div className="text-xs text-gray-400">所有資料存於本機裝置，不會上傳到任何伺服器</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="bg-gray-50 rounded-xl py-3">
              <div className="text-lg font-bold text-gray-800">{data.accounts.length}</div>
              <div className="text-xs text-gray-400">帳戶</div>
            </div>
            <div className="bg-gray-50 rounded-xl py-3">
              <div className="text-lg font-bold text-gray-800">{data.cards.length}</div>
              <div className="text-xs text-gray-400">信用卡</div>
            </div>
            <div className="bg-gray-50 rounded-xl py-3">
              <div className="text-lg font-bold text-gray-800">{(data.transactions || []).length}</div>
              <div className="text-xs text-gray-400">交易紀錄</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="font-medium text-gray-800 text-sm">清除所有資料</div>
              <div className="text-xs text-gray-400">刪除所有帳戶、信用卡及交易紀錄</div>
            </div>
            <button onClick={clearAllData} className="text-xs text-red-500 hover:text-red-600 font-medium px-3 py-1.5 rounded-lg hover:bg-red-50 transition cursor-pointer">
              清除
            </button>
          </div>
        </div>
