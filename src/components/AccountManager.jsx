    ...
    <div>
      ...
      {accounts.length === 0 ? (
        ...
        <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {accounts.map((acc, i) => (
            <div key={acc.id} className={`${ACCOUNT_BG[i % ACCOUNT_BG.length]} rounded-2xl p-4 border border-white/60 hover:shadow-md transition group relative overflow-hidden`}>
              <div className="absolute -right-4 -bottom-4 opacity-[0.06]">
                <IconBank className="w-20 h-20" />
              </div>
              <div className="relative">
                <div className="flex items-start justify-between mb-2">
                  <div className={`${ICON_BG[i % ICON_BG.length]} w-8 h-8 rounded-lg flex items-center justify-center text-white`}>
                  ...
                  <div className="flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition">
                    ...
                    <button onClick={() => startEdit(acc)} className="w-7 h-7 rounded-lg bg-white/80 hover:bg-white flex items-center justify-center text-gray-400 hover:text-gray-600 cursor-pointer transition" title="編輯"><IconEdit /></button>
                    <button onClick={() => remove(acc.id)} className="w-7 h-7 rounded-lg bg-white/80 hover:bg-white flex items-center justify-center text-gray-400 hover:text-red-500 cursor-pointer transition" title="刪除"><IconTrash /></button>
                  </div>
                </div>
                <div className="font-semibold text-gray-800 text-sm">{acc.bank}</div>
                {acc.lastFour && <div className="text-[11px] text-gray-400 font-mono">---- {acc.lastFour}</div>}
                <div className="mt-2">
                  <div className="text-lg font-bold text-gray-800">${(Number(acc.balance) || 0).toLocaleString()}</div>
                  <div className="text-[10px] text-gray-400">餘額</div>
                </div>
                {acc.purpose && <span className="inline-block text-[10px] bg-white/60 text-gray-600 px-2 py-0.5 rounded-full mt-1.5">{acc.purpose}</span>}
                {acc.note && <p className="text-gray-400 text-[11px] mt-1 truncate">{acc.note}</p>}
              </div>
            </div>
          ))}
        </div>
